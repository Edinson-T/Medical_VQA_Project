...
def _get_args():
    ...
    parser.add_argument('-c',
                        '--checkpoint-path',
                        type=str,
                        default='./results/Qwen3.5_0.8b/best_model',
...
def _launch_demo(args, model, processor, backend):
    ...
    def call_local_model(model, processor, messages, backend):
        ...
        if backend == 'vllm':
            ...
            gen_kwargs = {'max_new_tokens': 256,   #Original max_new_tokens is 1024, we set it to 256
                          'streamer': streamer, 
                          'do_sample': False,          
    ...
    def create_predict_fn():
        ...
        def predict(_chatbot, task_history):
            ...
            for response in call_local_model(model, processor, messages, backend):
                _chatbot[-1]["content"] = _remove_image_special(_parse_text(response))
                yield _chatbot
