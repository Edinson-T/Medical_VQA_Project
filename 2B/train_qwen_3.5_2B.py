"""
train_qwen3.5_2B_unsloth.py

Differences vs the QLoRA version:
  1. Model loading  →  FastVisionModel (Unsloth), not AutoModelForImageTextToText
  2. LoRA application →  FastVisionModel.get_peft_model with
                          use_gradient_checkpointing="unsloth"
  3. BitsAndBytesConfig / prepare_model_for_kbit_training removed
     (Unsloth handles 4-bit internally)
  4. max_pixels unified to MAX_PIXELS in ALL three places that use it
     (bug fix: full_inputs and prompt_inputs must use the same value
      or the label masking is offset by the image-token difference)
  5. enable_thinking=False added to model.generate() to suppress
     Qwen3.5's chain-of-thought tokens during validation generation

Installation (run once):
  pip install unsloth
  # If triton fails on Windows try:
...
# ── Image resolution ─────────────────────────────────────────────────────────
# 64 patches × 28×28 px = 50,176 px total.
# This is the safe lower bound for 2B on 8 GB VRAM.
# If VRAM allows, try 112 first; fall back to 64 if OOM.
# CRITICAL: MAX_PIXELS must be identical for full_inputs AND prompt_inputs
# inside MedicalVQADataset.__getitem__ — a mismatch shifts the label mask.
MAX_PIXELS = 112 * 28 * 28

# Unsloth needs an explicit max_seq_length.
# With 64 image patches + a typical question/answer, 1024 is sufficient;
# increase to 2048 if you see truncation warnings.
MAX_SEQ_LENGTH = 1024

SEED = 42

LOCAL_JSON_PATH = r"D:\1MA2Semester\ML&BDP\Medical_vqa_project\VQA_RAD_Dataset_Public.json"
OUTPUT_DIR     = "./results/Qwen3.5_2B"
CHECKPOINT_DIR = "./checkpoints/Qwen3.5_2B"
...
print("=" * 80)
print("Medical VQA  —  Unsloth LoRA Training  (Qwen3.5-2B)")
print("=" * 80)
